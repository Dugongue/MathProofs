...
PermanentDeterminantalLowerBound_CharZero_AllSizes.lean - A proof that every size-\(m\) affine determinantal representation of the \(n\times n\) permanent over any characteristic-zero field satisfies \(n^2\le 2m\), strengthened to \(n^2+1\le 2m\) whenever \(n\ge3\). The result completely subsumes Mignon–Ressayre’s Theorem 2 from A quadratic bound for the determinant and permanent problem (2004), recovered as an explicit corollary. It improves the integer lower bound by one for every even \(n\ge4\), while matching the original rounded bound for odd sizes.
