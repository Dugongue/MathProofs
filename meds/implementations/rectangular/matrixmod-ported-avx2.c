...
/* Comparison port of IIS-summer-2023/meds-simd-lowlevel's eight-column
 * broadcast/mullo/add AVX2 multiplication. Adapted from 32-bit to the 2024
 * uint16_t representation. Safe scalar tails replace upstream out-of-bounds
 * full-vector tail reads. Reduction and row kernels match our candidate. */
void pmod_mat_mul(pmod_mat_t *C, int C_r, int C_c, pmod_mat_t *A, int A_r, int A_c, pmod_mat_t *B, int B_r, int B_c)
{
  GFq_t tmp[C_r*C_c];uint16_t bits=0;
  ...
  if(A_c<=256 && (bits>>12)==0){
    int c=0;
    for(;c+8<=C_c;c+=8)for(int r=0;r<C_r;++r){
      __m256i acc=_mm256_setzero_si256();
      for(int k=0;k<A_c;++k){
        __m256i aa=_mm256_set1_epi32(A[r*A_c+k]);
        __m256i bb=_mm256_cvtepu16_epi32(_mm_loadu_si128((const __m128i*)(B+k*B_c+c)));
        acc=_mm256_add_epi32(acc,_mm256_mullo_epi32(aa,bb));
      }
      uint32_t x[8];_mm256_storeu_si256((__m256i*)x,acc);
      for(int j=0;j<8;++j)tmp[r*C_c+c+j]=reduce4093(x[j]);
    }
    for(;c<C_c;++c)for(int r=0;r<C_r;++r){
      uint32_t acc=0;for(int k=0;k<A_c;++k)acc+=(uint32_t)A[r*A_c+k]*B[k*B_c+c];
      tmp[r*C_c+c]=reduce4093(acc);
    }
  }else{
    for(int r=0;r<C_r;++r)for(int c=0;c<C_c;++c){
      uint64_t acc=0;for(int k=0;k<A_c;++k)acc+=(uint64_t)A[r*A_c+k]*B[k*B_c+c];
