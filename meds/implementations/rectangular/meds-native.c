/* Experimental modification: remove three unused inversions; see REPORT.md. */
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
    ...
    unsigned char *sk)
{
  ...
  for (int i = 1; i < MEDS_s; i++)
  {
    ...
    while (1 == 1) // redo generation for this index until success
    {
      ...
      pmod_mat_t G0prime[2 * MEDS_m * MEDS_n];

      pmod_mat_mul(G0prime, 2, MEDS_m * MEDS_n,
                   Ti, 2, MEDS_k,
                   G[0], MEDS_k, MEDS_m * MEDS_n);

      LOG_MAT(G0prime, 2, MEDS_m * MEDS_n);
      ...
      /* A^{-1} = first_m_columns(C0 * B), by matrix cancellation.
       * B has n-column row stride even though only m columns are needed. */
      pmod_mat_mul(A_inv[i], MEDS_m, MEDS_m,
                   G0prime, MEDS_m, MEDS_n, B, MEDS_n, MEDS_n);
    ...
    const unsigned char *sk)
{
  ...
  for (int i = 0; i < MEDS_t; i++)
  {
    ...
    while (1 == 1)
    {
      ...
      /* Redundant: A*C0 = [I|0]*B_inv, and B_inv is already invertible. */
