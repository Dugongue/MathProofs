...
/-!
# Tensor isomorphism and symplectic tensor isomorphism

Chen, Grochow, Qiao, Tang, and Zhang asked whether TI_Sp equals TI in
Open Question 8 of their ITCS 2024 paper (Question 1.8 in the full version).
Their Theorem 4 (Theorem 1.4 in the full version) proves Sp3TI -> GL3TI,
using bilinear-form constraints and the tensor-system construction of
Futorny, Grochow, and Sergeichuk. That direction is prior work.

Our missing-direction construction is GL3TI -> Sp3TI by hyperbolic padding.
Each factor U is embedded in U plus its dual; every side length doubles.
The classical lift P -> P plus P^(-transpose) preserves the pairing.
Orbit reflection follows from covariance of intrinsic mode supports:
restrict a symplectic transporter to the active primal supports, then extend
these restrictions to invertible maps of the original factors. Conciseness
is unnecessary, and the argument works over every field, including characteristic 2.

Both reductions, their correctness, and polynomial output-size bounds are
proved below. `ti_eq_tisp` establishes equality under the coordinate-projection
reduction system defined here. Uniform Turing-machine running-time bounds
are outside this formalization; the published computational interpretation
uses the field/model conventions of the cited paper.

Main results: `gl_iff_padded`, `generalToSymplectic`, `symplecticToGeneral`,
and `ti_eq_tisp`. Only Mathlib is imported.
...
Actions by Classical Groups, ITCS 2024, LIPIcs 287, 31:1-31:23.
Theorem 4, p. 31:6; Open Question 8, p. 31:10.
...
theorem stdJ_entry_src (n : ℕ) (i j : SymplecticIndex n) :
    ∃ σ : Src (SymplecticIndex n × SymplecticIndex n × SymplecticIndex n), ∀ A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K, stdJ K n i j = (evalSource A σ) := by
  classical
...
theorem Hgadget_src (n : ℕ) {th : ℕ} (strH : Fin th → Fin 4)
    ...
    ∃ σ : Src (SymplecticIndex n × SymplecticIndex n × SymplecticIndex n),
      ∀ A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K,
        Hgadget (K := K) n strH e0 e1 e2 A k b s i j = (evalSource A σ) := by
  classical
  cases b <;> cases s
...
noncomputable def packedGadget (n : ℕ) (A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K) :
...
theorem H_src (n : ℕ)
    ...
    ∃ σ : Src (SymplecticIndex n × SymplecticIndex n × SymplecticIndex n),
      ∀ A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K,
        packedGadget (K := K) n A z.1 z.2.1.1 z.2.2.1 z.2.1.2 z.2.2.2 = (evalSource A σ) :=
...
theorem H_anchor_entry (n : ℕ) (A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K) (i j : Unit) :
    packedGadget (K := K) n A (k3 n) Blk.anch Blk.anch i j = 1 := by
  rw [packedGadget, Hgadget_anch_anch, if_pos (strHn_k3 n)]

theorem H_anchor_entry_off (n : ℕ) (A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K)
    {k : Fin (thn n)} (h : strHn n k ≠ 3) (i j : Unit) :
    packedGadget (K := K) n A k Blk.anch Blk.anch i j = 0 := by
  rw [packedGadget, Hgadget_anch_anch, if_neg h]
...
theorem H_anchor_block (n : ℕ) (A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K) :
    packedGadget (K := K) n A (k3 n) Blk.anch Blk.anch = 1 :=
  unit_matrix_ext (by rw [H_anchor_entry]; simp [Matrix.one_apply])

theorem H_anchor_block_off (n : ℕ) (A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K)
    {k : Fin (thn n)} (h : strHn n k ≠ 3) :
    packedGadget (K := K) n A k Blk.anch Blk.anch = 0 :=
  unit_matrix_ext (by rw [H_anchor_entry_off n A h]; simp)

theorem anchor_scalar_prod (n : ℕ) (A B : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K)
    (P Q : ∀ b : Blk, Matrix (BIdx n b) (BIdx n b) K)
    (R : Matrix (Fin (thn n)) (Fin (thn n)) K)
    (heq : ∀ k b s, packedGadget (K := K) n B k b s
      = ∑ k', R k k' • (P b * packedGadget (K := K) n A k' b s * (Q s)ᵀ)) :
    R (k3 n) (k3 n) * (P Blk.anch () ()) * (Q Blk.anch () ()) = 1 := by
  classical
  have hsum : (∑ k', R (k3 n) k' • (P Blk.anch * packedGadget (K := K) n A k' Blk.anch Blk.anch
        * (Q Blk.anch)ᵀ))
...
theorem anchor_normalization (n : ℕ) (A B : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K)
    (h : BlockEquiv (strHn n) (packedGadget (K := K) n A) (packedGadget (K := K) n B)) :
    ∃ (P Q : ∀ b : Blk, Matrix (BIdx n b) (BIdx n b) K)
      (R : Matrix (Fin (thn n)) (Fin (thn n)) K),
      (∀ b, IsUnit (P b).det) ∧ (∀ s, IsUnit (Q s).det) ∧ IsUnit R.det ∧
      IsBlockDiag (strHn n) R ∧
      P Blk.anch = 1 ∧ Q Blk.anch = 1 ∧ R (k3 n) (k3 n) = 1 ∧
      (∀ k b s, packedGadget (K := K) n B k b s
        = ∑ k', R k k' • (P b * packedGadget (K := K) n A k' b s * (Q s)ᵀ)) := by
  classical
...
theorem H_dat_mid_on (n : ℕ) (A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K) :
    packedGadget (K := K) n A (k3 n) Blk.dat Blk.mid = 1 := by
  ext i j
  rw [packedGadget, Hgadget_dat_mid, if_pos (strHn_k3 n), Matrix.one_apply]

theorem H_dat_mid_off (n : ℕ) (A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K)
    {k : Fin (thn n)} (h : strHn n k ≠ 3) :
    packedGadget (K := K) n A k Blk.dat Blk.mid = 0 := by
  ext i j
  rw [packedGadget, Hgadget_dat_mid, if_neg h]
  simp

theorem H_mid_cop_on (n : ℕ) (A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K) :
    packedGadget (K := K) n A (k3 n) Blk.mid Blk.cop = 1 := by
  ext i j
  rw [packedGadget, Hgadget_mid_cop, if_pos (strHn_k3 n), Matrix.one_apply]

theorem H_mid_cop_off (n : ℕ) (A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K)
    {k : Fin (thn n)} (h : strHn n k ≠ 3) :
    packedGadget (K := K) n A k Blk.mid Blk.cop = 0 := by
  ext i j
  rw [packedGadget, Hgadget_mid_cop, if_neg h]
  simp

theorem H_cop_dat_on (n : ℕ) (A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K) :
    packedGadget (K := K) n A (k3 n) Blk.cop Blk.dat = stdJ K n := by
  ext i j
  rw [packedGadget, Hgadget_cop_dat, if_pos (strHn_k3 n)]

theorem H_cop_dat_off (n : ℕ) (A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K)
    {k : Fin (thn n)} (h : strHn n k ≠ 3) :
    packedGadget (K := K) n A k Blk.cop Blk.dat = 0 := by
  ext i j
  rw [packedGadget, Hgadget_cop_dat, if_neg h]
  simp

theorem H_dat_anch_on (n : ℕ) (A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K)
    (k : Fin (thn n)) (h : strHn n k = 1) (i : BIdx n Blk.dat) (u : BIdx n Blk.anch) :
    packedGadget (K := K) n A k Blk.dat Blk.anch i u = stdJ K n i ((e1n n).symm ⟨k, h⟩) := by
  rw [packedGadget, Hgadget_dat_anch, dif_pos h]

theorem H_dat_anch_off (n : ℕ) (A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K)
    {k : Fin (thn n)} (h : strHn n k ≠ 1) :
    packedGadget (K := K) n A k Blk.dat Blk.anch = 0 := by
  ext i u
  rw [packedGadget, Hgadget_dat_anch, dif_neg h]
  simp

theorem H_mid_anch_on (n : ℕ) (A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K)
    (k : Fin (thn n)) (h : strHn n k = 0) (i : BIdx n Blk.mid) (u : BIdx n Blk.anch) :
    packedGadget (K := K) n A k Blk.mid Blk.anch i u
      = (1 : Matrix (SymplecticIndex n) (SymplecticIndex n) K) i ((e0n n).symm ⟨k, h⟩) := by
  rw [packedGadget, Hgadget_mid_anch, dif_pos h, Matrix.one_apply]

theorem H_mid_anch_off (n : ℕ) (A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K)
    {k : Fin (thn n)} (h : strHn n k ≠ 0) :
    packedGadget (K := K) n A k Blk.mid Blk.anch = 0 := by
  ext i u
  rw [packedGadget, Hgadget_mid_anch, dif_neg h]
  simp

theorem H_cop_anch_on (n : ℕ) (A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K)
    (k : Fin (thn n)) (h : strHn n k = 2) (i : BIdx n Blk.cop) (u : BIdx n Blk.anch) :
    packedGadget (K := K) n A k Blk.cop Blk.anch i u
      = (1 : Matrix (SymplecticIndex n) (SymplecticIndex n) K) i ((e2n n).symm ⟨k, h⟩) := by
  rw [packedGadget, Hgadget_cop_anch, dif_pos h, Matrix.one_apply]

theorem H_cop_anch_off (n : ℕ) (A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K)
    {k : Fin (thn n)} (h : strHn n k ≠ 2) :
    packedGadget (K := K) n A k Blk.cop Blk.anch = 0 := by
  ext i u
  rw [packedGadget, Hgadget_cop_anch, dif_neg h]
  simp

theorem H_anch_cop_on (n : ℕ) (A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K)
    (k : Fin (thn n)) (h : strHn n k = 0) (u : BIdx n Blk.anch) (j : BIdx n Blk.cop) :
    packedGadget (K := K) n A k Blk.anch Blk.cop u j = stdJ K n j ((e0n n).symm ⟨k, h⟩) := by
  rw [packedGadget, Hgadget_anch_cop, dif_pos h]

theorem H_anch_cop_off (n : ℕ) (A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K)
    {k : Fin (thn n)} (h : strHn n k ≠ 0) :
    packedGadget (K := K) n A k Blk.anch Blk.cop = 0 := by
  ext u j
  rw [packedGadget, Hgadget_anch_cop, dif_neg h]
  simp

theorem H_anch_mid_on (n : ℕ) (A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K)
    (k : Fin (thn n)) (h : strHn n k = 1) (u : BIdx n Blk.anch) (j : BIdx n Blk.mid) :
    packedGadget (K := K) n A k Blk.anch Blk.mid u j
      = (1 : Matrix (SymplecticIndex n) (SymplecticIndex n) K) j ((e1n n).symm ⟨k, h⟩) := by
  rw [packedGadget, Hgadget_anch_mid, dif_pos h, Matrix.one_apply]

theorem H_anch_mid_off (n : ℕ) (A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K)
    {k : Fin (thn n)} (h : strHn n k ≠ 1) :
    packedGadget (K := K) n A k Blk.anch Blk.mid = 0 := by
  ext u j
  rw [packedGadget, Hgadget_anch_mid, dif_neg h]
  simp

theorem H_anch_dat_on (n : ℕ) (A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K)
    (k : Fin (thn n)) (h : strHn n k = 2) (u : BIdx n Blk.anch) (j : BIdx n Blk.dat) :
    packedGadget (K := K) n A k Blk.anch Blk.dat u j
      = (1 : Matrix (SymplecticIndex n) (SymplecticIndex n) K) j ((e2n n).symm ⟨k, h⟩) := by
  rw [packedGadget, Hgadget_anch_dat, dif_pos h, Matrix.one_apply]

theorem H_anch_dat_off (n : ℕ) (A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K)
    {k : Fin (thn n)} (h : strHn n k ≠ 2) :
    packedGadget (K := K) n A k Blk.anch Blk.dat = 0 := by
  ext u j
  rw [packedGadget, Hgadget_anch_dat, dif_neg h]
  simp

theorem symplectic_of_normalised (n : ℕ) (A B : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K)
    (P Q : ∀ b : Blk, Matrix (BIdx n b) (BIdx n b) K)
    (R : Matrix (Fin (thn n)) (Fin (thn n)) K)
  ...
  have h11 : Coupling (1 : Matrix (SymplecticIndex n) (SymplecticIndex n) K) (P Blk.dat) (Q Blk.mid) :=
    anchorSlice_coupling Blk.dat Blk.mid 1 (packedGadget n A) (packedGadget n B) (H_dat_mid_on n A)
  ...
  have h32 : Coupling (1 : Matrix (SymplecticIndex n) (SymplecticIndex n) K) (P Blk.mid) (Q Blk.cop) :=
    anchorSlice_coupling Blk.mid Blk.cop 1 (packedGadget n A) (packedGadget n B) (H_mid_cop_on n A)
      (fun _ hk => H_mid_cop_off n A hk) (H_mid_cop_on n B) P Q R hR1 heq

  have h23' : Coupling (stdJ K n) (P Blk.cop) (Q Blk.dat) :=
    anchorSlice_coupling Blk.cop Blk.dat (stdJ K n) (packedGadget n A) (packedGadget n B) (H_cop_dat_on n A)
      (fun _ hk => H_cop_dat_off n A hk) (H_cop_dat_on n B) P Q R hR1 heq

  have h13 : Coupling (stdJ K n) (P Blk.dat) (Rres (e1n n) R) :=
    stratumRow_coupling Blk.dat (e1n n) (stdJ K n) (packedGadget n A) (packedGadget n B)
  ...
  have h31' : Coupling (1 : Matrix (SymplecticIndex n) (SymplecticIndex n) K) (P Blk.mid) (Rres (e0n n) R) :=
    stratumRow_coupling Blk.mid (e0n n) 1 (packedGadget n A) (packedGadget n B)
  ...
  have h22' : Coupling (1 : Matrix (SymplecticIndex n) (SymplecticIndex n) K) (P Blk.cop) (Rres (e2n n) R) :=
    stratumRow_coupling Blk.cop (e2n n) 1 (packedGadget n A) (packedGadget n B)
  ...
  have h33' : Coupling (stdJ K n) (Q Blk.cop) (Rres (e0n n) R) :=
    stratumCol_coupling Blk.cop (e0n n) (stdJ K n) (packedGadget n A) (packedGadget n B)
  ...
  have h12 : Coupling (1 : Matrix (SymplecticIndex n) (SymplecticIndex n) K) (Q Blk.mid) (Rres (e1n n) R) :=
    stratumCol_coupling Blk.mid (e1n n) 1 (packedGadget n A) (packedGadget n B)
  ...
  have h21 : Coupling (1 : Matrix (SymplecticIndex n) (SymplecticIndex n) K) (Q Blk.dat) (Rres (e2n n) R) :=
    stratumCol_coupling Blk.dat (e2n n) 1 (packedGadget n A) (packedGadget n B)
...
theorem H_dat_dat_on (n : ℕ) (A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K)
    (k : Fin (thn n)) (h : strHn n k = 0) (i j : BIdx n Blk.dat) :
    packedGadget (K := K) n A k Blk.dat Blk.dat i j = A (i, j, (e0n n).symm ⟨k, h⟩) := by
  rw [packedGadget, Hgadget_dat_dat, dif_pos h]

theorem H_dat_dat_off (n : ℕ) (A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K)
    {k : Fin (thn n)} (h : strHn n k ≠ 0) :
    packedGadget (K := K) n A k Blk.dat Blk.dat = 0 := by
  ext i j
  rw [packedGadget, Hgadget_dat_dat, dif_neg h]
  simp

theorem data_equation (n : ℕ) (A B : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K)
    (P Q : ∀ b : Blk, Matrix (BIdx n b) (BIdx n b) K)
    (R : Matrix (Fin (thn n)) (Fin (thn n)) K)
    (heq : ∀ k b s, packedGadget (K := K) n B k b s
      = ∑ k', R k k' • (P b * packedGadget (K := K) n A k' b s * (Q s)ᵀ)) :
  ...
  rw [Matrix.sum_apply] at h
  have hterm : ∀ k' : Fin (thn n),
      (R (e0n n x).1 k' • (P Blk.dat * packedGadget (K := K) n A k' Blk.dat Blk.dat * (Q Blk.dat)ᵀ)) i j
        = R (e0n n x).1 k'
            * ∑ j', (∑ i', P Blk.dat i i' * packedGadget (K := K) n A k' Blk.dat Blk.dat i' j')
                * Q Blk.dat j j' := by
    intro k'
  ...
  have hre : (∑ c : Fib (strHn n) 0, R (e0n n x).1 c.1
        * ∑ j', (∑ i', P Blk.dat i i' * packedGadget (K := K) n A c.1 Blk.dat Blk.dat i' j')
            * Q Blk.dat j j')
    ...
    have hin : ∀ j' i' : BIdx n Blk.dat,
        P Blk.dat i i' * A (i', j', y)
          = P Blk.dat i i' * packedGadget (K := K) n A (e0n n y).1 Blk.dat Blk.dat i' j' := by
      intro j' i'
...
theorem sp3ti_of_blockEquiv_H (n : ℕ) (A B : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K)
    (h : BlockEquiv (strHn n) (packedGadget (K := K) n A) (packedGadget (K := K) n B)) :
...
theorem isUnit_det_of_symplectic {n : ℕ} {g : Matrix (SymplecticIndex n) (SymplecticIndex n) K} (h : IsSymplecticMat K g) :
    IsUnit g.det := by
...
def colJ {J : Type} (m : SymplecticIndex n) : Matrix (SymplecticIndex n) J K := Matrix.of fun i _ => stdJ K n i m

def rowJ {I : Type} (m : SymplecticIndex n) : Matrix I (SymplecticIndex n) K := Matrix.of fun _ j => stdJ K n j m
...
theorem mul_colJ_entry (g : Matrix (SymplecticIndex n) (SymplecticIndex n) K) (m i : SymplecticIndex n) (u : J) :
    (g * colJ K (J := J) m) i u = ∑ i', g i i' * stdJ K n i' m := by
  rw [Matrix.mul_apply]
  simp [colJ]

theorem rowJ_mul_entry (g : Matrix (SymplecticIndex n) (SymplecticIndex n) K) (m j : SymplecticIndex n) (u : I) :
...
theorem cov_entry {g : Matrix (SymplecticIndex n) (SymplecticIndex n) K} (h : g * stdJ K n * gᵀ = stdJ K n) (i x : SymplecticIndex n) :
...
theorem colJ_transport {g : Matrix (SymplecticIndex n) (SymplecticIndex n) K} (h : g * stdJ K n * gᵀ = stdJ K n) (x : SymplecticIndex n) :
...
theorem rowJ_transport {g : Matrix (SymplecticIndex n) (SymplecticIndex n) K} (h : g * stdJ K n * gᵀ = stdJ K n) (x : SymplecticIndex n) :
...
theorem Hmat_dat_mid :
    Hg A k Blk.dat Blk.mid = if strH k = 3 then (1 : Matrix (SymplecticIndex n) (SymplecticIndex n) K) else 0 := by
...
theorem Hmat_mid_cop :
    Hg A k Blk.mid Blk.cop = if strH k = 3 then (1 : Matrix (SymplecticIndex n) (SymplecticIndex n) K) else 0 := by
...
theorem blockEquiv_of_sp3ti (g₁ g₂ g₃ : Matrix (SymplecticIndex n) (SymplecticIndex n) K)
...
theorem sp3ti_rel_imp_blockEquiv_H (n : ℕ) (A B : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K)
    (h : (Sp3TI K).Rel n A B) : BlockEquiv (strHn n) (packedGadget n A) (packedGadget n B) := by
...
theorem H_srcExpr (n : ℕ) : SrcExpr (fun A : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K => toFun (packedGadget n A)) :=
  fun z => H_src (K := K) n z
...
noncomputable def sp3tiProjection
    (hH : ∀ n (A B : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K),
      (Sp3TI K).Rel n A B ↔ BlockEquiv (strHn n) (packedGadget n A) (packedGadget n B)) :
    Projection (Sp3TI K) (GL3TI K) :=
  projectionOfEncoding (Sp3TI K) (fun n => BIdx n) (fun n => BIdx n) thn (fun _ => 4) strHn
    (fun n A => packedGadget n A) hH H_srcExpr ⟨Kcomp Blk Blk 4 ^ 21 * 22 ^ 24, 8, Nside_H_bound⟩
...
theorem sp3ti_iff_blockEquiv_H (n : ℕ) (A B : SymplecticIndex n × SymplecticIndex n × SymplecticIndex n → K) :
    (Sp3TI K).Rel n A B ↔ BlockEquiv (strHn n) (packedGadget (K := K) n A) (packedGadget (K := K) n B) :=
  ⟨sp3ti_rel_imp_blockEquiv_H n A B, sp3ti_of_blockEquiv_H n A B⟩

noncomputable def symplecticToGeneral : Projection (Sp3TI K) (GL3TI K) :=
  sp3tiProjection
    (fun n A B => sp3ti_iff_blockEquiv_H n A B)

end EncodingEquivalence

/-- Ordinary and symplectic three-tensor isomorphism generate the same class
under polynomial-size coordinate projections, over every field. -/
theorem ti_eq_tisp (K : Type*) [Field K] :
    problemClass (projectionSystem K) (GL3TI K) =
      problemClass (projectionSystem K) (Sp3TI K) :=
  problemClass_eq_of_reduces_both (projectionSystem K)
    ⟨generalToSymplectic⟩ ⟨symplecticToGeneral⟩

end TISp

#print axioms TISp.gl_iff_padded
#print axioms TISp.generalToSymplectic
#print axioms TISp.symplecticToGeneral
#print axioms TISp.ti_eq_tisp
